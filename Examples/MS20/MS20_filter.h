...
class MS20_Filter
{
  public:
    ...
    inline sample process(const int mode, const int nonlinearity, const int clippingMode, sample x)
    {
      ...
      switch (nonlinearity)
      {
        case MS20_Expensive: { Nonlinearity = MS20_Nonlinearities::nonlinear_tanh; break; }
        case MS20_Cheap: { Nonlinearity = MS20_Nonlinearities::nonlinear_fasttanh; break; }
        case MS20_Crossover: { Nonlinearity = MS20_Nonlinearities::nonlinear_crossover; break; }
        case MS20_Broken: { Nonlinearity = MS20_Nonlinearities::nonlinear_broken; break; }
        case MS20_Asymmetric: { Nonlinearity = MS20_Nonlinearities::nonlinear_asym; break; }
      }

      std::tuple<sample, sample>(*Diode)(sample);
      switch (clippingMode)
      {
        case MS20_Soft: { Diode = MS20_Nonlinearities::diode_slow; break; }
        case MS20_Clip: { Diode = MS20_Nonlinearities::diode_clip; break; }
        case MS20_Asym: { Diode = MS20_Nonlinearities::diode_asym; break; }
        case MS20_Rezz: { Diode = MS20_Nonlinearities::diode_rezzy; break; }
      }

      switch (mode)
      {
        case MS20_Lowpass: return lowpass(x, Diode, Nonlinearity);
        case MS20_Bandpass: return bandpass(x, Diode, Nonlinearity);
        case MS20_Highpass: return highpass(x, Diode, Nonlinearity);
        case MS20_Bandreject: return bandreject(x, Diode, Nonlinearity);
        default:
          return 0.0;
      };
    }
